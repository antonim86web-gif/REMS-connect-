...
<style>
    ...
    /* --- STILE POST-IT --- */
    .postit { padding: 15px; border-radius: 8px; margin-bottom: 12px; border-left: 10px solid; box-shadow: 2px 2px 5px rgba(0,0,0,0.1); color: #1e293b; background-color: #ffffff; position: relative; }
    .postit-header { font-weight: 800; font-size: 0.85rem; text-transform: uppercase; margin-bottom: 5px; display: flex; justify-content: space-between; padding-right: 30px; }
...
def render_postits(p_id=None, limit=20, can_delete=False):
    ...
    for d, r, o, nt, uid in res:
        cls = f"role-{r.lower()}"
        col_post, col_del = st.columns([0.9, 0.1])
        with col_post:
            st.markdown(f'''
            ...
            ''', unsafe_allow_html=True)
        if can_delete:
            if col_del.button("🗑️", key=f"ev_{uid}"):
    ...
    with tab_reg:
        with st.form("form_reg"):
            ...
            if st.form_submit_button("INVIA REGISTRAZIONE"):
                if db_run("SELECT * FROM utenti WHERE user=?", (nu,)): st.warning("Username occupato")
                else:
                    db_run("INSERT INTO utenti VALUES (?,?,?,?,?)", (nu, hash_pw(np), nn, nc, nq), True)
                    st.success("Registrato! Accedi ora."); st.rerun()
    st.stop()
...
if nav == "📊 Monitoraggio":
    ...
    t1, t2 = st.tabs(["👥 UTENTI", "🌍 LOG ATTIVITÀ (MODIFICA)"])
    with t1:
    ...
    with t2:
        st.info("In questa sezione puoi eliminare i singoli post-it cliccando sul cestino.")
