...
<style>
    ...
    /* FORZA SCRITTE BIANCHE NEL MENU RADIO E TESTI */
    div[data-testid="stSidebar"] .stRadio label, 
    div[data-testid="stSidebar"] .stRadio div[role="radiogroup"] p,
    ...
    /* TASTO LOGOUT - ROSSO VIVO E TESTO BIANCO */
...
if nav == "📊 Monitoraggio Generale":
    ...
    if p_lista:
        ...
        if u['ruolo'] == "Psichiatra":
                ...
                for tid, fa, do in ter:
                    with st.expander(f"📌 {fa} ({do})", expanded=True):
                        ...
                        if c_btn.button("REGISTRA", key=f"btn_{tid}"):
                            s = "✔️ SOMM." if c_ass.checkbox("A", key=f"a_{tid}") else "❌ RIF."
                            db_run("INSERT INTO eventi (id, data, nota, ruolo, op) VALUES (?,?,?,?,?)", (p_id, d_s, f"{s}: {fa}", "Infermiere", firma), True); st.rerun()
            with t2:
            ...
            with tc:
                ...
                with st.form("c"):
                    ...
                    if st.form_submit_button("SALVA"):
                        db_run("INSERT INTO cassa (p_id, data, causale, importo, tipo, op) VALUES (?,?,?,?,?,?)", (date.today().strftime("%d/%m/%Y"), c, i, t, firma, p_id), True); st.rerun()
            with te:
                nota = st.text_area("Nota Attività")
            ...
            if st.button("REGISTRA"): db_run("INSERT INTO eventi (id, data, nota, ruolo, op) VALUES (?,?,?,?,?)", (p_id, datetime.now().strftime("%d/%m/%Y %H:%M"), f"🛠️ {m_s}: {n_o}", "OSS", firma), True); st.rerun()
    ...
    with st.form("ag"):
        ...
        d_app, o_app, cat = c1.date_input("Data"), c2.text_input("Ora"), c3.selectbox("Tipo", ["Uscita", "Visita Medica", "Udienza", "Parenti"])
    ...
    for pid, n in db_run("SELECT id, nome FROM pazienti ORDER BY nome"):
        ...
        c1.write(f"👤 {n}")
        if c2.button("X", key=f"d_{pid}"): db_run("DELETE FROM pazienti WHERE id=?", (pid,), True); st.rerun()
