    ...
    if ruolo != "Scegli...":
        ...
        if p_lista:
            ...
            if ruolo == "Psichiatra":
                ...
                st.write("**Gestione Terapie Attive:**")
                ...
                t1, t2 = st.tabs(["📊 Parametri Vitali", "📝 Consegne"])
                with t1:
                    with st.form("pv"):
                ...
                with t2:
                    ...
                    if st.button("SALVA CONSEGNA"):
                        db_run("INSERT INTO eventi (id,data,umore,nota,ruolo,op) VALUES (?,?,?,?,?,?)", (p_id, datetime.now().strftime("%d/%m %H:%M"), "Stabile", f"📝 {txt}", "Infermiere", f_i), True); st.rerun()
                ...
                st.metric("SALDO ATTUALE CASSA", f"€ {saldo:.2f}")
                with st.form("cassa"):
                    tp, im, ds = st.radio("Operazione", ["Entrata", "Uscita"]), st.number_input("Importo €", min_value=0.0), st.text_input("Causale")
