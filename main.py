...
# --- 1. CONFIGURAZIONE PAGINA E DESIGN FINALE ---
st.set_page_config(
...
<style>
    ...
    .card-box { background-color: #fcfcfc; border-radius: 10px; padding: 20px; margin-bottom: 15px; border: 1px solid #e2e8f0; color: #1e293b !important; }
...
if menu == "📊 Monitoraggio":
    ...
    if ruolo == "Scegli...": st.info("👈 Seleziona il tuo profilo nella sidebar.")
        ...
        if p_lista:
            ...
            if ruolo == "Psichiatra":
                f_med = st.text_input("Firma Medico (Obbligatoria)")
                with st.form("p_f"):
                ...
                ter = db_run("SELECT data_prescr, farmaco, dosaggio, turni FROM terapie WHERE p_id=?", (p_id,))
                if ter:
                    st.write("Terapie in Corso:")
                    html = "<table class='custom-table'><tr><th>Data</th><th>Farmaco</th><th>Dose</th><th>Turni</th></tr>"
                    for dt, fa, ds, tu in ter: html += f"<tr><td>{dt}</td><td>{fa}</td><td>{ds}</td><td>{tu}</td></tr>"
                ...
                with t3:
                    txt = st.text_area("Note Cliniche / Consegne")
                    if st.button("SALVA NOTA LIBERA"):
                ...
                if mov:
                    html = "<table class='custom-table'><tr><th>Data</th><th>Causale</th><th>E</th><th>U</th><th>Operatore</th></tr>"
    ...
    if paz:
        ...
        with st.form("ag"):
            ...
            ti = st.selectbox("Tipo", ["Udienza", "Visita", "Permesso"]); acc = st.text_input("Scorta Incaricata")
        ...
        apps = db_run("SELECT data, ora, tipo, scorta, row_id FROM appuntamenti WHERE p_id=? ORDER BY row_id DESC", (p_id,))
        if apps:
            html = "<table class='custom-table'><tr><th>Data/Ora</th><th>Tipo</th><th>Scorta</th><th>Az.</th></tr>"
            for d, o, t, sc, rid in apps:
                html += f"<tr><td>{d} {o}</td><td>{t}</td><td>{sc}</td><td>"
    ...
    nuovo = st.text_input("Inserire Nuovo Nome")
    ...
    for idx, nome in db_run("SELECT id, nome FROM pazienti"):
        ...
        c1.write(f"**{nome}** (ID: {idx})")
        if c2.button("Elimina", key=f"p_{idx}"): db_run("DELETE FROM pazienti WHERE id=?", (idx,), True); st.rerun()
