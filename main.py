...
if menu == "Gestione":
    ...
    with t1:
        with st.form("add_p"):
            ...
            if st.form_submit_button("SALVA"):
                if nuovo_nome: db_run("INSERT INTO pazienti (nome) VALUES (?)", (nuovo_nome,), True); st.rerun()
    with t2:
        paz_list = db_run("SELECT id, nome FROM pazienti ORDER BY nome")
        if paz_list:
            p_da_mod = st.selectbox("Paziente da modificare", [p[1] for p in paz_list])
    ...
    with t3:
        ...
        if paz_list_del:
            p_da_del = st.selectbox("Paziente da eliminare", [p[1] for p in paz_list_del])
...
# --- 6. EQUIPE ---
    ...
    if paz_data:
        ...
        if figura == "Psichiatra":
            ...
            med_firma = st.text_input("Firma Medico (Obbligatoria)")
            
            with st.expander("➕ Prescrivi Farmaco"):
                with st.form("new_p"):
                    f, d = st.text_input("Farmaco"), st.text_input("Dosaggio")
                    c1,c2,c3 = st.columns(3)
                    tm, tp, tn = c1.checkbox("M"), c2.checkbox("P"), c3.checkbox("N")
                    if st.form_submit_button("REGISTRA"):
                        if not med_firma: st.error("Firma obbligatoria!"); st.stop()
            ...
            st.write("#### 💊 Riepilogo Terapia Attiva")
            terapie = db_run("SELECT data_prescr, farmaco, dosaggio, turni, medico, row_id FROM terapie WHERE p_id=? ORDER BY row_id DESC", (p_id,))
            if terapie:
                h = "<table class='custom-table'><tr><th>DATA</th><th>FARMACO</th><th>DOSE</th><th>TURNI</th><th>MEDICO</th><th>AZIONE</th></tr>"
                st.markdown(h, unsafe_allow_html=True)
                for da, fa, do, tu, me, rid in terapie:
                    col1, col2, col3, col4, col5, col6 = st.columns([1.5, 2, 1, 1, 1.5, 1])
                    col1.write(da); col2.markdown(f"**{fa}**"); col3.write(do); col4.write(tu); col5.write(me)
                    if col6.button("🗑️ Sospendi", key=f"s_{rid}"):
        ...
        # --- INFERMIERE ---
            ...
            c_data, c_turno = st.columns(2)
            d_s = c_data.date_input("Data", date.today())
            t_s = c_turno.selectbox("Turno", ["Mattina", "Pomeriggio", "Notte"])
            
            terapie = db_run("SELECT farmaco, dosaggio, turni, row_id FROM terapie WHERE p_id=?", (p_id,))
            for f, d, tu_f, rid in terapie:
                if tu_f and t_s[0] in tu_f:
                    st.markdown("<div class='card-box'>", unsafe_allow_html=True)
                    c_a, c_b, c_c = st.columns([2, 2, 1])
                    c_a.write(f"**{f}** ({d})")
                    scelta = c_b.radio("Esito", ["Assunta", "Rifiutata"], key=f"som_{rid}", horizontal=True)
                    if c_c.button("CONVALIDA", key=f"btn_{rid}"):
                        if not inf_firma: st.error("Firma obbligatoria!"); st.stop()
                        db_run("INSERT INTO eventi (id,data,umore,nota,ruolo,op) VALUES (?,?,?,?,?,?)", 
                               (p_id, f"{d_s.strftime('%d/%m/%Y')} {datetime.now().strftime('%H:%M')}", "Stabile", f"[{t_s[0]}] {f} -> {scelta}", "Infermiere", inf_firma), True); st.rerun()
        ...
        # --- EDUCATORE ---
            ...
            st.markdown(f'<div class="saldo-box">Saldo Attuale: € {saldo:.2f}</div>', unsafe_allow_html=True)
            
            with st.expander("Nuovo Movimento"):
                t_mov = st.radio("Tipo", ["Entrata", "Uscita"], horizontal=True)
                i_mov = st.number_input("Cifra €", min_value=0.0, step=0.5)
                d_mov = st.text_input("Causale")
                if st.button("REGISTRA TRANSAZIONE"):
                    if not ed_firma: st.error("Firma obbligatoria!"); st.stop()
                    db_run("INSERT INTO soldi (p_id, data, desc, importo, tipo, op) VALUES (?,?,?,?,?,?)", (p_id, date.today().strftime("%d/%m/%Y"), d_mov, i_mov, t_mov, ed_firma), True); st.rerun()
            ...
            if movs:
                ...
                for d, ds, im, tp, op in movs:
                    ...
                    h += f"<tr><td>{d}</td><td>{ds}</td><td>{e}</td><td>{u}</td><td>{op}</td></tr>"
        ...
        # --- OSS ---
            ...
            with st.form("oss_f"):
                ...
                if st.form_submit_button("REGISTRA ATTIVITÀ"):
                    ...
                    if ms:
                        db_run("INSERT INTO eventi (id,data,umore,nota,ruolo,op) VALUES (?,?,?,?,?,?)", (p_id, datetime.now().strftime("%d/%m/%Y %H:%M"), "Stabile", f"🧹 Mansioni: {', '.join(ms)}", "OSS", f_oss), True); st.rerun()
