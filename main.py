...
def db_query(query, params=(), commit=False):
    ...
    # AGGIORNAMENTO SCHEMA: Controlla e aggiunge le colonne se mancano
...
menu = st.sidebar.radio("VAI A:", ["Monitoraggio", "Gestione"])

# --- 5. MONITORAGGIO ---
if menu == "Monitoraggio":
    st.title("Diario di Turno")
    pazienti = db_query("SELECT id, nome FROM pazienti ORDER BY nome")
    
    if not pazienti:
        st.info("Nessun paziente. Aggiungili in 'Gestione'.")
    ...
    for p_id, nome in pazienti:
        with st.expander(f"👤 {nome.upper()}"):
            ...
            nota = st.text_area("Nota:", key=f"n_{p_id}_{st.session_state[f'v_{p_id}']}", height=120)
            ...
            if st.button("SALVA NOTA", key=f"btn_{p_id}"):
                if nota and operatore:
                    ...
                    st.session_state[f"v_{p_id}"] += 1 # Reset campi
                    st.rerun()
                else:
                    st.error("Firma e Nota obbligatorie!")

            st.divider()
            
            # LETTURA DIARIO - Protezione estrema contro dati corrotti
            eventi = db_query("SELECT data, umore, nota, ruolo, operatore FROM eventi WHERE p_id=? ORDER BY id DESC", (p_id,))
            
            if not eventi:
                st.write("Nessuna nota presente.")
            
            for e in eventi:
                # Gestione sicura dei dati (se sono nulli li sostituiamo al volo)
                d_data = str(e[0]) if e[0] else "Data N.D."
                d_umore = str(e[1]) if e[1] else "N.D."
                d_nota = str(e[2]) if e[2] else ""
                d_ruolo = str(e[3]) if e[3] else "Nota"
                d_op = str(e[4]) if e[4] else "Anonimo"
                
                # Applica colore in base al ruolo
                classe_ruolo = f"nota-{d_ruolo.replace(' ', '')}"
                
                st.markdown(f"""
                <div class="nota-card {classe_ruolo}">
                    <small><b>{d_data}</b> | <b>{d_ruolo.upper()}</b> | {d_op}</small><br>
                    <b>Stato: {d_umore}</b><br>
                    <div style="margin-top:5px; white-space: pre-wrap;">{d_nota}</div>
                </div>
                """, unsafe_allow_html=True)
