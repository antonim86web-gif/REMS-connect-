...
with col_nav1:
    if st.button("📊 Monitoraggio"): 
        ...
        st.rerun()
with col_nav2:
    # Solo admin vede il tasto gestione attivo, per gli altri è bloccato
...
if menu == "📊 Monitoraggio":
    ...
    for p_id, nome in pazienti:
        with st.expander(f"👤 {nome.upper()}", expanded=False):
            ...
            umore = st.select_slider("Stato del Paziente:", options=["Cupo", "Deflesso", "Stabile", "Agitato"], value="Stabile", key=f"u_{p_id}_{st.session_state[f'v_{p_id}']}")
            nota = st.text_area("Nota di Turno:", key=f"n_{p_id}_{st.session_state[f'v_{p_id}']}", height=100)
            ...
            if eventi_raw:
                ...
                st.download_button(label="📥 SCARICA DIARIO (Excel/CSV)", data=csv, file_name=f"diario_{nome}_{datetime.now().strftime('%Y%m%d')}.csv", mime='text/csv')
            ...
            for data_label, note_del_giorno in diario_per_data.items():
                with st.expander(f"📅 Diario del {data_label}"):
                    ...
                    filtro_f = st.multiselect(f"Filtra per:", ruoli_presenti, key=f"f_{p_id}_{data_label}")
                    
                    for n in note_del_giorno:
                        if filtro_f and n['ruolo'] not in filtro_f: continue
    ...
    if p_list:
        with st.expander("✏️ MODIFICA"):
            ...
            if st.button("AGGIORNA"):
                db_query("UPDATE pazienti SET nome=? WHERE nome=?", (nuovo_n, p_da_mod), commit=True); st.rerun()
