...
if st.session_state.menu_val == "📊 Monitoraggio":
    ...
    for p_id, nome in pazienti:
        with st.expander(f"👤 {nome.upper()}", expanded=False):
            ...
            # Semplificazione chiavi per evitare SyntaxError
    ...
    paz = db_query("SELECT id, nome FROM pazienti ORDER BY nome")
    p_dict = {p[1]: p[0] for p in paz}
    with st.expander("➕ REGISTRA NUOVO EVENTO"):
        p_sel = st.selectbox("Paziente:", list(p_dict.keys()))
        tipo = st.selectbox("Tipo:", ["Visita con Parenti", "Uscita con Operatore", "Udienza / Perizia", "Visita Medica Esterna", "Altro"])
        d_ev = st.date_input("Data:")
        o_ev = st.time_input("Ora:")
        acc = st.text_input("Riferimento / Accompagnatore:")
        det = st.text_area("Note:")
        if st.button("PROGRAMMA"):
            dt_s = f"{d_ev} {o_ev.strftime('%H:%M')}"
            db_query("INSERT INTO agenda (p_id, tipo, data_ora, note, operatore_rif) VALUES (?,?,?,?,?)", (p_dict[p_sel], tipo, dt_s, det, acc), True)
            st.rerun()
    st.divider()
    agenda = db_query("SELECT agenda.tipo, agenda.data_ora, agenda.note, agenda.operatore_rif, pazienti.nome FROM agenda JOIN pazienti ON agenda.p_id = pazienti.id ORDER BY agenda.data_ora ASC")
    for ev in agenda:
        st.markdown(f'<div class="agenda-card"><b>{ev[0].upper()}</b> - {ev[1]}<br>Paziente: <b>{ev[4]}</b><br><small>Rif: {ev[3]} | Note: {ev[2]}</small></div>', unsafe_allow_html=True)

elif st.session_state.menu_val == "⚙️ Gestione" and st.session_state.role == "admin":
    st.title("Gestione")
    with st.expander("➕ Aggiungi"):
        n_n = st.text_input("Nome")
        if st.button("Salva"): db_query("INSERT INTO pazienti (nome) VALUES (?)", (n_n,), True); st.rerun()
    p_l = db_query("SELECT id, nome FROM pazienti ORDER BY nome")
    if p_l:
        with st.expander("🗑️ Elimina"):
            p_d = st.selectbox("Paziente", [p[1] for p in p_l])
            if st.button("Elimina"): db_query("DELETE FROM pazienti WHERE nome=?", (p_d,), True); st.rerun()
