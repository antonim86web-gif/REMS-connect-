...
<style>
    ...
    /* AGENDA FULL SCREEN PER PORTATILE */
    .agenda-container { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; background-color: #cbd5e1; border: 2px solid #1e3a8a; border-radius: 8px; overflow: hidden; margin-top: 10px; }
    .day-header { background-color: #1e3a8a; color: white; text-align: center; padding: 12px; font-weight: 800; text-transform: uppercase; font-size: 0.8rem; }
    .day-cell { background-color: white; min-height: 150px; padding: 8px; position: relative; border: 1px solid #f1f5f9; vertical-align: top; }
    .day-num { font-weight: 900; color: #64748b; font-size: 0.9rem; margin-bottom: 8px; border-bottom: 1px solid #eee; display: block; }
    .today-cell { background-color: #f0fdf4 !important; border: 2px solid #22c55e !important; box-shadow: inset 0 0 10px rgba(34,197,94,0.2); }
    
    .ev-tag { font-size: 0.75rem; padding: 4px 8px; border-radius: 4px; margin-bottom: 4px; border-left: 5px solid; line-height: 1.3; font-weight: 600; box-shadow: 1px 1px 3px rgba(0,0,0,0.1); }
    .ev-programmato { background: #dbeafe; color: #1e40af; border-left-color: #2563eb; }
    .ev-completato { background: #f1f5f9; color: #94a3b8; border-left-color: #cbd5e1; text-decoration: line-through; }
    .ev-ora { font-weight: 800; color: #1e3a8a; margin-right: 5px; }
...
# --- 📅 MODULO AGENDA DINAMICA (VERSIONE PORTATILE) ---
if nav == "📅 Agenda Dinamica":
    ...
    with cm:
        ...
        html = "<div class='agenda-container'>"
        for h in gh: html += f"<div class='day-header'>{h}</div>"
        for week in calendar.Calendar(0).monthdayscalendar(st.session_state.cal_year, st.session_state.cal_month):
            for d in week:
                if d == 0: html += "<div class='day-cell' style='background:#f8fafc;'></div>"
                else:
                    cur = f"{st.session_state.cal_year}-{st.session_state.cal_month:02d}-{d:02d}"
                    cls = "today-cell" if cur == oggi_iso else ""
                    tags = "".join([f"<div class='ev-tag {'ev-programmato' if e['s']=='PROGRAMMATO' else 'ev-completato'}' title='{e['n']}'><span class='ev-ora'>{e['o']}</span>{e['p']}</div>" for e in mev.get(d, [])])
                    html += f"<div class='day-cell {cls}'><span class='day-num'>{d}</span>{tags}</div>"
