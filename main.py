...
# 2. STILE CSS (Ottimizzato per leggere la firma)
st.markdown("""
    <style>
    ...
    .oggi { border: 2px solid #1e3a8a !important; background: #fffde7 !important; }
...
str_mese_anno = oggi_dt.strftime("%m/%Y")
gg_mese = calendar.monthrange(oggi_dt.year, oggi_dt.month)[1]
...
if not p_list:
    ...
    tab1, tab2 = st.tabs(["💊 REGISTRO", "🩺 MEDICO"])
            ...
            if any(f[idx] == 1 for idx in mappa[turno]):
                ...
                with st.popover(f"✍️ Firma per OGGI"):
                    ...
                    if c1.button("✅ ASSUNTO", key=f"ok_{f[0]}"):
                        dt_f = f"{giorno_oggi:02d}/{str_mese_anno} {datetime.now().strftime('%H:%M')}"
