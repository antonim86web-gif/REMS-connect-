...
# --- 2. TERAPIE (SOMMINISTRAZIONE + ELIMINAZIONE) ---
    ...
    if u['ruolo'] == "Psichiatra":
        ...
        with t1:
            ...
            attive = db_query("""
                SELECT t.id_t, p.nome, t.farmaco, t.dose, t.mat, t.pom, t.nott 
                FROM terapie t JOIN pazienti p ON t.p_id = p.id 
                WHERE t.stato='ATTIVO' ORDER BY p.nome
            """)
            if attive:
                for tid, p_nome, farm, dose, m, p, n in attive:
                    ...
                    if c3.button("❌ Elimina", key=f"del_{tid}"):
                        ...
                        db_query("INSERT INTO eventi (p_id, data, nota, ruolo, op, categoria) VALUES (?,?,?,?,?,?)",
                                 (pid, datetime.now().strftime("%d/%m/%Y %H:%M"), f"🚫 SOSPESA TERAPIA: {farm}", u['ruolo'], firma, "Terapia"), True)
                        st.rerun()
...
# --- 3. DIARIO CLINICO ---
    ...
    p_sel = st.selectbox("Paziente", [p[1] for p in db_query("SELECT id, nome FROM pazienti WHERE stato='ATTIVO'")])
