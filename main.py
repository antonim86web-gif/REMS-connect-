...
if nav == "📊 Monitoraggio":
    ...
    if p_lista:
        ...
        # --- INFERMIERE ---
            ...
            with t1:
                ...
                def card_s(tid, farm, dos, turn, icon):
                    ...
                    if not check:
                        st.markdown(f"<div class='therapy-container'><div class='turn-header'>{icon} {turn}</div><b>{farm}</b> - {dos}</div>", unsafe_allow_html=True)
                        if st.button(f"CONFERMA {turn} - {farm}", key=f"btn_{tid}_{turn}"):
        ...
        # --- EDUCATORE ---
            ...
            with t1:
                with st.form("ed_c"):
                    n = st.text_area("Consegne del Giorno")
                    if st.form_submit_button("SALVA"):
