    ...
    with t_reg:
        with st.form("r"):
            ...
            if st.form_submit_button("REGISTRA"):
                db_run("INSERT INTO utenti VALUES (?,?,?,?,?)", (new_u, hash_pw(new_p), new_n, new_c, new_q), True); st.success("Ok!"); st.rerun()
    st.stop()
        ...
        with t2:
            with st.form("ed_con"):
                ...
                if st.form_submit_button("INVIA NOTA"):
                    db_run("INSERT INTO eventi (id, data, nota, ruolo, op) VALUES (?,?,?,?,?)", (p_id, adesso_str, f"📝 NOTA EDUC.: {cns}", "Educatore", firma_op), True); st.rerun()
        ...
        with t2:
            with st.form("oss_con"):
                ...
                if st.form_submit_button("INVIA CONSEGNA"):
                    db_run("INSERT INTO eventi (id, data, nota, ruolo, op) VALUES (?,?,?,?,?)", (p_id, adesso_str, f"📝 CONSEGNA OSS: {cns}", "OSS", firma_op), True); st.rerun()
