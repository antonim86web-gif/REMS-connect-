        ...
        with st.expander(f"👤 {nome.upper()}", expanded=False):
            ...
            if log:
                ...
                for d, r, o, n, u in log:
                    h += f"<tr><td>{d}</td><td>{r}</td><td>{o}</td><td>{u}</td><td>{n}</td></tr>"
        ...
        if p_lista:
            ...
            if ruolo == "Psichiatra":
                ...
                with t1:
                    ...
                    for fa, do, tu, rid in db_run("SELECT farmaco, dosaggio, turni, id_u FROM terapie WHERE p_id=?", (p_id,)):
                        if turno[0] in tu:
                            ...
                            if c2.button("✔️", key=f"a_{rid}"): db_run("INSERT INTO eventi (id,data,umore,nota,ruolo,op) VALUES (?,?,?,?,?,?)", (p_id, datetime.now().strftime("%d/%m %H:%M"), "Stabile", f"💊 Assunto: {fa}", "Infermiere", f_i), True); st.rerun()
                            if c3.button("❌", key=f"r_{rid}"): db_run("INSERT INTO eventi (id,data,umore,nota,ruolo,op) VALUES (?,?,?,?,?,?)", (p_id, datetime.now().strftime("%d/%m %H:%M"), "Stabile", f"💊 Rifiutato: {fa}", "Infermiere", f_i), True); st.rerun()
                    st.write("**Storico:**")
                    for d, nt in db_run("SELECT data, nota FROM eventi WHERE id=? AND nota LIKE '💊 %' ORDER BY id_u DESC LIMIT 10", (p_id,)): 
                        st.markdown(f"<div class='report-box report-infermiere'>{d} - {nt}</div>", unsafe_allow_html=True)
                with t2:
                    with st.form("pv"):
                ...
                with t_oss2:
                    ...
                    for d, nt, op in db_run("SELECT data, nota, op FROM eventi WHERE id=? AND ruolo='OSS' AND nota LIKE '📝 %' ORDER BY id_u DESC", (p_id,)): 
                        st.markdown(f"<div class='report-box report-oss'><b>{d}</b> - {nt} (<i>{op}</i>)</div>", unsafe_allow_html=True)
